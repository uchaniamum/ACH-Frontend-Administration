...
export function useAnalyticsOptions() {
    ...
    const externalParticipantsFilterOptions = ref<{ label: string; value: string }[]>([]);

    const externalParticipantAcronymsOptions = ref<{ label: string; value: string }[]>([]);
    const externalParticipantAcronymsFilterOptions = ref<{ label: string; value: string }[]>([]);
    ...
    const loadperiodsOptions = async () => {
      try {
        ...
        periodsOptions.value =
          filtersService.mapPeriodsToSelectOptions(periods);
      } catch (err) {
