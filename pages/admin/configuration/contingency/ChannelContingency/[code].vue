...
const isFormContingencyValid = computed(() => {
    ...
    //     // Para contingencia también validar título y mensaje
    //     return baseValidation && 
    //         titleContingency.value?.trim() && 
    //         messageContingency.value?.trim()
    // }
    
    // Para cambio de canal solo justificación
    return baseValidation
})
...
const openChannelChangeModal = (): void => {
    const entityText = contingencyData.value.length > 1 
        ...
        : `${contingencyData.value.length} entidad`
    ...
    confirmDialog.value = {
        ...
        options: {
            ...
            confirmLabel: 'Guardar',
            cancelLabel: 'Cancelar',
            message: `¿Estás seguro de cambiar de canal <span class="font-semibold">${entityText}</span>? Esta modificación podrá ser ajustada en el futuro.`,
            onConfirm: async () => {
                await saveConfirm()
            },
            onCancel: () => {
...
const openContingencyModal = (): void => {
    confirmDialog.value = {
        ...
        options: {
            ...
            confirmLabel: 'Guardar',
            cancelLabel: 'Cancelar',
            message: `¿Estás seguro de guardar la configuración para <span class="font-semibold">${getBankNames.value}</span>? Esta modificación podrá ser ajustada en el futuro.`,
            onConfirm: async () => {
                await saveConfirm()
            },
            onCancel: () => {
