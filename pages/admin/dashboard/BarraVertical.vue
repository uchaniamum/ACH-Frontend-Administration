<template>
  ...
  >
    ...
    <span
      v-if="copiado"
      class="absolute top-2 right-2 bg-blue-500 text-white text-sm px-2 py-1 rounded z-20"
    >
      Copiado
    </span>
...
// --- Plugin para mostrar valores individuales y sumatoria ---
const mostrarValoresPlugin = {
  ...
  afterDatasetsDraw(chart) {
    ...
    chart.data.labels.forEach((_, i) => {
      ...
      // sumatoria sobre QR
      const qrBar = chart.getDatasetMeta(0).data[i]
      ctx.save()
      ctx.fillStyle = "#373F4A"
      ctx.font = "9px Work Sans"
      ctx.textAlign = "center"
      ctx.textBaseline = "bottom"
      ctx.fillText(sum + "M", qrBar.x, qrBar.y - 6)
      ctx.restore()
    })
  },
}
